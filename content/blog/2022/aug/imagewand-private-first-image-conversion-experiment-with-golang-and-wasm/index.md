...
Golang offers some standard libraries for image manipulation ([`image` package](https://pkg.go.dev/image)). Implementing something to convert A to B wasn't the hard part. [The conversion logic was extracted and re-used](https://github.com/brunoluiz/imagewand/blob/f50156801e4ce55a29366303ee668c8c295a7960/imagewand.go) in both CLI and WASM implementations, which is impressive in terms of portability.
...
You can instantiate and load the WASM binary for every call, similar to a CLI call, but it would be an expensive operation. The best way is to run it in the background ([observe the use of a channel to keep it running](https://github.com/brunoluiz/imagewand/blob/f50156801e4ce55a29366303ee668c8c295a7960/cmd/wasm/main.go#L77)) while developers call it through a JavaScript interface.

In Golang, this is done by [exposing global JavaScript objects/functions within `main()`.](https://github.com/brunoluiz/imagewand/blob/f50156801e4ce55a29366303ee668c8c295a7960/cmd/wasm/main.go#L70-L78) It is not great as it might conflict with other variables and it pollutes the global scope. Other WASM implementations (Rust or TinyGo/WASI) [expose methods by exporting them](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly/Instance/exports) in the WASM instance, which is cleaner than relying on global variables.
...
To declare and interact with JavaScript in Golang runtime, developers rely on [`syscall/js`](https://pkg.go.dev/syscall/js). This package allows the application to create functions, read values from JS objects (DOM and non-DOM) and interface byte arrays with JS values (`UInt8Array`). The only caveat is the following, been present since its release on Golang 1.11.

_> This package is EXPERIMENTAL. Its current scope is only to allow tests to run, but not yet to provide a comprehensive API for users. It is exempt from the Go compatibility promise._

Although it allows all sorts of interactions with JavaScript, in my limited experience with it, I would recommend developers to use it only to interface user inputs between Golang and the JavaScript runtime.
...
I've [created an internal package (jasm)](https://github.com/brunoluiz/imagewand/blob/main/jasm/wasm.go) with a few helper functions to interface with JavaScript values. [It made `wasm/main.go` way cleaner](https://github.com/brunoluiz/imagewand/blob/main/cmd/wasm/main.go#L45-L68), but certainly the most important of all is the `Await` method.
...
The first thing I tried was to compile the program using TinyGo, without any changes. It incredibly worked quite well, and the binary size dropped from ~8Mb to ~500Kb. Quite an improvement, especially when the WASM target is a web browser.
...
[There are some hacks for it](https://github.com/tinygo-org/tinygo/issues/1140), but they all have memory leak issues. As the surface between the program functionality and `syscall/js` is quite limited, [changing those into `int` seemed a better solution (without any caveat)](https://github.com/brunoluiz/imagewand/commit/4860cb4c757c0ad859912047eb145798d47ff434).
...
As TinyGo supports WASI. This means  `syscall/js` is not required if you don't want to manipulate the DOM, which would have made ImageWand code simpler, becoming just an exported Golang function… But, there are a couple of limitations when implementing WASI programs with TinyGo:
...
3. Lacking documentation and helper methods to operate on top of shared WASM memory
...
These limitations and lack of documentation around WASI, workarounds and common examples for TinyGo make it quite hard to abandon `syscall/js` for browser apps or use it for non-JS WASM applications. Although I managed to make it work, I wouldn't recommend someone to use TinyGo for non-browser applications at the moment, only if the person wants to deep dive and study its details. If you decide to do so, please help the community by adding examples, tutorials and documentation to the official project page.
...
* TinyGo is an amazing alternative, but you might fall into edge-cases or unsupported features. This is quite an annoyance but, if you manage to get your way around it, it is probably the perfect choice for your WASM project.
* CI pipelines will get more complex [as Golang/TinyGo tooling needs to be handled](https://github.com/brunoluiz/imagewand/blob/f50156801e4ce55a29366303ee668c8c295a7960/package.json#L12-L15), and [extra build steps are required](https://github.com/brunoluiz/imagewand/blob/f50156801e4ce55a29366303ee668c8c295a7960/package.json#L9-L11). Not that JavaScript toolchains are not complex by themselves already
